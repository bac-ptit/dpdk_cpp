...
- When writing or reviewing code, apply ALL rules from these rule files:
  - `.claude/commands/dpdk-rules.md` — DPDK-specific lifecycle, error handling, resource management
  - `.claude/commands/performance-rules.md` — zero-overhead abstraction, noexcept, attributes, memory
  - `.claude/commands/code-style-rules.md` — Google C++ naming, braces, formatting, includes
  - `.claude/commands/modern-cpp-rules.md` — brace init, if-init, expected, format
...
3. Performance — unnecessary copies? Missing `noexcept`? Missing `reserve()`?
